...
class Parser:
    ...
    def if_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'ЕСЛИ'):
        ...
        while self.current_tok.matches(TT_KEYWORD, 'ИНАСЛИ'):
            res.register_advancement()
            self.advance()
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Ожидали целое число, число с плавающей точкой, индентифекатор, '+', '—', или '('"))
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'ПЕРЕМ'):
            ...
            if self.current_tok.type != TT_IDENTIFIER:
                return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Ожидали идентификатор"))
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Ожидали 'ПЕРЕМ', целое число, число с плавающей точкой, индентифекатор, '+', '—', '(', или 'НЕ'"))
...
class Number:
    ...
    def dived_by(self, other):
        if isinstance(other, Number):
            if other.value == 0:
                return None, RTError(other.pos_start, other.pos_end, 'Деление на ноль', self.context)
...
class Interpreter:
    ...
    def no_visit_method(self, node, context):
        raise Exception(f'visit_{type(node).__name__} метод не определён')
